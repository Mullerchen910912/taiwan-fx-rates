...
- **Precious metals** (`scraper/metals.py`): international spot gold/silver/platinum/palladium from [gold-api.com](https://www.gold-api.com/) (no key), converted to an approximate NT$/gram using the same run's median USD→TWD. Shown as a clearly-labelled *reference* panel — explicitly **not** a bank gold-passbook (黃金存摺) quote, with a link to Bank of Taiwan's passbook board.
...
出國前想知道**去哪家銀行換外幣最划算**?牌告匯率相同的兩家銀行,手續費可以差到 NT$500;匯率聚合網站上「最便宜」的那家,牌價可能已經三週沒更新(日期藏在 HTML 註解裡)。本站直接排名**含手續費的總台幣成本**、自動標記過舊牌價,每天 09–21 點每 2 小時自動更新。資料來源為比率網(各銀行牌告);線上結匯與外幣 ATM 常有額外減碼,實際成交以各銀行為準。
